...
def toy_example(satopt, train_loader, val_loader, test_loader, ckpt_path=None):
    ...
    model.to(device)
    optimizer = torch.optim.Adam(model.parameters(),lr=satopt["training"]["optamizer"]["lr"],weight_decay=satopt["training"]["optamizer"]["weight_decay"])
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=satopt["training"]["epoch"],eta_min=1e-7)
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(SATDataLoader(train_dataset, cutoff=0.1, k_hop=1),batch_size=batch_size,shuffle=True,num_workers=0,collate_fn=collate_fn)
    val_loader = DataLoader(SATDataLoader(val_dataset, cutoff=0.1, k_hop=1),batch_size=batch_size,shuffle=True,num_workers=0,collate_fn=collate_fn)
    test_loader = DataLoader(SATDataLoader(test_dataset, cutoff=0.1, k_hop=1),batch_size=batch_size,shuffle=False,num_workers=0,collate_fn=collate_fn)
