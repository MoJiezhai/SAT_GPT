...
class SATGPT(nn.Module):
    def __init__(self, num_atom_types, k_dim=0, d_model=256, n_layer=6, n_head=8, d_head=32, d_ff=512, coord_emb_dim=128, max_length=128):
        super().__init__()
        self.d_model = d_model
        self.k_dim = k_dim
        ...
        self.prop_head = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Linear(d_model, 9)   # 3 lattice vectors
        )
        ...
        self.film = nn.Sequential(
            nn.Linear(k_dim, 2 * self.d_model),
            nn.SiLU(),
            nn.Linear(2 * self.d_model, 2 * self.d_model)
        )

    def forward(self, Zs, coords, lengths,cond=None, subgraph_node_index=None, subgraph_indicator=None, sub_batch_index=None, sat=False):
        """
        Zs: [B, L] (atom types)
        coords: [B, L, 3]
        ...
        x = atom_e + coord_e
        if cond is not None:#FiLM调制
            film_params = self.film(cond)
            gamma, beta = film_params.chunk(2, dim=-1)

            gamma = 1 + gamma.unsqueeze(1)
        ...
        #预测晶格
        # ===== Lattice prediction from CLS =====
        cls_repr = x_struct[:, 0]        # [B, d_model]
        lattice_pred = self.prop_head(cls_repr)   # [B, 9]

        lattice_pred = lattice_pred.view(-1, 9)  # reshape to matrix


        # typically for autoreg: the target for position t is Z_{t+1}, r_{t+1}; so we may shift when computing losses
        return coord_preds, lattice_pred#, {"struct": attn_maps_struct }
    ...
    def sample(self, Zs, cond=None, max_len=None):
        ...
        prop_gen = torch.zeros(B, 9 , device=device)
        ...
        for t in range(L):
            ...
            coord_preds, prop_preds = self.forward(
                Zs,
                coords_in,
