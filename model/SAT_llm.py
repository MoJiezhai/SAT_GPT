...
class SATGPT(nn.Module):
    def __init__(self, num_atom_types, k_dim=0, d_model=256,
        ...
        self.index_emb_atom = nn.Embedding(max_length, d_model)
        # optional：区分 atom token 类型
        self.axis_emb_condition = nn.Embedding(4, d_model)
        # optional: structural stream tag
        self.atom_group_emb = nn.Parameter(
    ...
    def _condition_(self, Zs, lattice, cond=None):
        ...
        return:
            context: [B, N_ctx, D]
            context_mask: [B, N_ctx]  (True = padding)
        """
        B, L = Zs.shape
        device = Zs.device
        ...
        # mask: Zs==0 视为 padding（你用的是 num_atom_types+1）
        atom_mask = (Zs == 0)                      # [B, L]
        
        

        # ===== 2. lattice token =====
        lattice_token = self.lattice_proj(lattice) # [B, D]
        lattice_token = self.ln_lattice(lattice_token)
        lattice_token = lattice_token.unsqueeze(1)  # [B, 1, D]
    ...
    def forward(self, Zs, coords, lengths, lattice,
        ...
        # ===== embedding =====
        emb = self.coord_embed(coord_idx)  # [B, T, D]
        # 👉 BOS
        ...
        def mask(lengths, B, L, device):
            ...
            key_mask = pad_mask[:, None, :].expand(-1, L, -1)  # [B, L, L]
            query_mask = pad_mask[:, :, None].expand(-1, -1, L)  # [B, L, L]
        ...
        # ===== GPT head =====
        logits = self.lm_head(x_struct)#为什么我们要重复操作x_struct
    ...
    def generate(self, Zs, lengths, lattice, cond=None,
        ...
        for t in range(T - 1):  # 逐 token 生成

            # ===== 2. padding 到 full 长度（为了 forward）=====
            ...
            # ===== 4. 取当前 step 的 logits =====
            logits_t = logits[:, coord_seq.shape[1]-1]  # 当前最后一个token位置
