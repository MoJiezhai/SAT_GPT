...
class GPTBlock(nn.Module):
    def __init__(self, d_model, n_head, d_head, d_ff, cond_dim=0, dropout=0.1):
        super().__init__()
        self.ln1 = nn.LayerNorm(d_model)
        self.attn = SATCausalAttention(d_model, n_head, d_head, dropout=dropout)
...
class SATGPT(nn.Module):
    ...
    def _condition_(self, Zs, lattice, cond=None):
        ...
        atom_tokens = self.atom_embed(Zs)  
        
        atom_indices = torch.arange(L, device=device).unsqueeze(0) 
        ...
        context[:, 0:1, :] += self.type_emb(torch.tensor(0, device=device)) 
        context[:, 1:1+L, :] += self.type_emb(torch.tensor(1, device=device))
        if cond is not None:
            context[:, -1:, :] += self.type_emb(torch.tensor(2, device=device)) 
