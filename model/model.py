...
class TransformerEncoderLayer(nn.TransformerEncoderLayer):
    ...
    def forward(self, x, cond=None, attn_mask=None, key_padding_mask=None, temperature=0.0):
        if cond is not None:
            B, L = cond.shape
            d_cond = self.d_cond
            if L < d_cond:
            ...
            if temperature > 0:
                gamma_beta1 = gamma_beta1 + temperature * torch.randn_like(gamma_beta1)
                gamma_beta2 = gamma_beta2 + temperature * torch.randn_like(gamma_beta2)
...
class TransformerEncoder(nn.TransformerEncoder):
    def forward(self, x, cond=None, attn_mask=None, key_padding_mask=None, temperature=0.0 ):
        for layer in self.layers:
            x = layer(x, cond,attn_mask=attn_mask, key_padding_mask=key_padding_mask, temperature=temperature)
        return x
...
class Transformer(nn.Module):
    def __init__(self,
        ...
        self.element_embed = nn.Embedding(num_elements+1, d_model, padding_idx=0)
    ...
    def forward(self, Z, cond=None, lengths=None, temperature=0.0):
        device = Z.device
        B, Lmax = Z.shape

        output = self.element_embed(Z) 
        if temperature > 0:
            output = output + temperature * torch.randn_like(output)
        ...
        if cond_tokens is not None:
            ...
            masks = [global_mask, cond_mask, atom_mask]
        else:
        ...
        if self.use_token_type:
            ...
            if cond_tokens is not None:
                type_ids[1:1+cond_tokens.size(1)] = 2 
                type_ids[1+cond_tokens.size(1):] = 1  
            else:
                type_ids[1:] = 1 
