...
class CrystalStructureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 对采样到的对象进行预处理
        #print(preprocess_object(sampled_object))
        sampled_object_new=preprocess_object(sampled_object)
#        sampled_object_new["dft_band_gap"] = row["dft_band_gap"]
        #print(sampled_object_new)
        return sampled_object_new
...
def parse_atoms(data_item):
    ...
    #print(data_item,cj)
    cond = [
        float(data_item.get("dft_band_gap", 0.0)),
...
def apply_random_shift(coords_frac, shift_scale=1.0, pbc=True):
    """
    对分数坐标进行全局随机平移增强。
    
    参数:
        coords_frac (torch.Tensor): 输入的分数坐标，Shape 为 [B, L, 3]
        shift_scale (float): 平移强度，范围 [0.0, 1.0]。
                             0.0 表示不平移，1.0 表示全空间随机平移。
        pbc (bool): 是否应用周期性边界条件。
        
    返回:
        shifted_coords (torch.Tensor): 平移后的坐标。
    """
    if shift_scale <= 0:
        return coords_frac, torch.zeros((coords_frac.shape[0], 1, 3), device=coords_frac.device)

    B, _, _ = coords_frac.shape
    device = coords_frac.device

    # 1. 生成 [-0.5, 0.5] 之间的随机数，然后缩放
    # 这样当 shift_scale 较小时，坐标是在原位置附近微调
    random_shift = (torch.rand((B, 1, 3), device=device) - 0.5) * shift_scale

    # 2. 应用平移
    shifted_coords = coords_frac + random_shift

    # 3. PBC 处理
    if pbc:
        # 使用 % 1.0 确保结果始终在 [0, 1) 之间
        shifted_coords = shifted_coords % 1.0
        
    return shifted_coords, random_shift