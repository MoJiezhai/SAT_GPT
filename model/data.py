...
class CrystalStructureDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.random and self.dim == 2:
            sampled_object = random.choice(variants) 
...
def parse_atoms(data_item):
    ...
    """
    seq_Ele = data_item["cryst_seq_ele"]  
...
class SATDataLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'prop': P,
            'par': par,
            "sub_nodes": torch.cat(all_sub_nodes),      
            "sub_indicator": torch.cat(all_indicator)   
        }
...
def data_add(train_ds):
    ...
    for data in (train_ds):    
        ...
        L = len(Z)
        pad =  torch.full((L, 3), -100.0)  
        coords = torch.tensor(coords, dtype=torch.float32)
        coords = torch.cat([pad, coords], dim=0).numpy()  
