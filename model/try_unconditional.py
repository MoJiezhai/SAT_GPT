            ...
            for batch in dataloader:
                ...
                if phase == "train" :#or phase == "test" or phase == "val":
                    ...
                    coord_preds = model.generate(
                        Zs=Z_seq, 
                        lengths=lengths_seq, 
                        lattice=prop_preds,   # 注意这里用预测的 lattice（更真实）
                        cond=cond)
                ...
                #loss_type = torch.tensor(0.0, device=device)
                loss = loss_type + prop_scale * loss_prop + coord_scale * loss_coord #+ rms_loss*total_scale
